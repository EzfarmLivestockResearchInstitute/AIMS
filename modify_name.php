...
$user_newname1=mysqli_real_escape_string($con, $_POST['n_name1']);
$user_newname2=mysqli_real_escape_string($con, $_POST['n_name2']);

if ( !$user_newname1) {//새로 설정할 이름은 공란이 될 수 없습니다.
	echo "<script>
	window.alert("."'".selectwords($con, $sessvar, 'a0231')."'".")
	history.go(-1)
	</script>";	
	exit(1);  
	}
elseif ( $user_newname1 != $user_newname2 ) {//변경할 이름이 일치하지 않습니다!
	echo "<script>
	window.alert("."'".selectwords($con, $sessvar, 'a0232')."'".")
	history.go(-1)
	</script>";	
	exit(1);  
	}
else {//회원님의 이름이 성공적으로 변경되었습니다.
	echo "<script>
...
mysqli_query($con,"UPDATE regist SET name='{$user_newname1}' WHERE id='{$user_id}'  ");
